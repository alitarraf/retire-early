...
// would ship unseen. renderToStaticMarkup runs the full render in node. `rec`
// comes from recommendedFunding (memoized in App), so we compute it here too.
...
const render = (plan, rec) => renderToStaticMarkup(<FundingOrderCard plan={plan} rec={rec} onApply={() => {}} />);
...
describe("FundingOrderCard render", () => {
  ...
  it("retired (rec unavailable): shows where the money sits, no cascade", () => {
    ...
    const rec = recommendedFunding(plan); // { available: false }
    const html = render(plan, rec);
  ...
  it("null rec (still computing): renders the balance view without crashing", () => {
